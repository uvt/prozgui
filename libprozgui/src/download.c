...
/* $Id: download.c,v 1.33 2001/09/30 23:13:50 kalum Exp $ */
/* $Id: download.c,v 1.34 2011/03/24 23:13:50 ahmad Exp $ */
						 ...
						 int req_connections)
{
  ...
  long long bytes_per_connection;
  ...
  snprintf(out_file, PATH_MAX, "%s/%s.prozgui",
  ...
  if (stat(out_file, &stat_buf) == -1)
    {
      ...
      */
      if (errno == ENOENT)
	{
		proz_debug("file does not exist");
  ...
  if ((proz_download_all_dls_status(download, COMPLETED)) == TRUE)
    {
      char * out_filename;
      char * orig_filename;
      download_show_message(download, _("All the connections have retrieved the file... waiting for them to end"));
      proz_download_wait_till_all_end(download);
      download_show_message(download, _("All the threads have being ended."));
      ...
      snprintf(out_filename, PATH_MAX, "%s/%s.prozgui",
	       download->dl_dir, download->pconnections[0]->u.file);
      if(rename(out_filename, orig_filename)==-1)
	{
	  download_show_message(download, _("Error while attempting to rename the file: %s"), strerror(errno));
	}

      download_show_message(download, _("Successfully renamed file"));
      /*Delete the logfile as we dont need it now */
      if(proz_log_delete_logfile(download)!=1)
	download_show_message(download, _("Error: Unable to delete the logfile: %s"), strerror(errno));
...
long long proz_download_get_total_bytes_got(download_t * download)
{
  long long total_bytes_got = 0;
  int i;
...
float proz_download_get_average_speed(download_t * download)
{
  ...
  long long total_remote_bytes_got =
...
******************************************************************************/
long long proz_download_get_total_remote_bytes_got(download_t * download)
{
...
*/
long long proz_download_get_est_time_left(download_t * download)
{

  long secs_left;
  float average_speed;
  long long total_bytes_got;
  ...
  return secs_left =
      (long long) ((download->main_file_size -
...
void download_calc_throttle_factor(download_t * download)
{
  ...
  /*MAKE IR USE THE NUMBER OF ACTIVE DOWNLOADING CONNECTIONS: Done */
  ...
  /*fixme mutex to prevent this connections */
...
Returns,
 1= sucessfully deleted the file
...
uerr_t download_handle_threads_ftpsearch(download_t * download)
{
  ...
  if ((proz_download_all_dls_status(download, COMPLETED)) == TRUE)
  {
    ...
    download_show_message(download,
			  "All the connections have retrieved the file... waiting for them to end");
    proz_download_wait_till_all_end(download);
