...
/* $Id: connection.c,v 1.42 2001/10/27 11:24:40 kalum Exp $ */
/* $Id: connection.c,v 1.43 2011/03/24 11:24:40 ahmad Exp $ */
				       ...
				       int read_buffer_size)
{
  ...
  do
  {
	...
	connection_show_message(connection, _("Unable to write to file %s: %s!"),
  ...
  connection_show_message(connection, _("Download for this connection completed"), connection->localfile,
				   ...
				   char *read_buffer, int read_buffer_size)
{
  ...
  while (bytes_to_get > 0)
  {
    ...
    {
      connection_show_message(connection, _("Server Closed Connection Prematurely!"));
      connection_change_status(connection, REMOTEFATAL);
      return READERR;
    }
...
int connection_load_resume_info(connection_t * connection)
{
  ...
  if(connection->remote_startpos-connection->orig_remote_startpos!=connection->remote_bytes_received)
	{
	  proz_debug("connection->remote start pos before loading %lld", connection->remote_startpos);
	  
	  connection->remote_startpos +=(connection->remote_bytes_received-(connection->remote_startpos-connection->orig_remote_startpos));	  

	  proz_debug("connection->remote start pos after loading %lld", connection->remote_startpos);
	}
  return 1;
}
...
by this connection.
******************************************************************************/
long long proz_connection_get_total_remote_bytes_got(connection_t * connection)
{
  long long ret;
  pthread_mutex_lock(&connection->access_mutex);
  ret = (connection->remote_bytes_received
