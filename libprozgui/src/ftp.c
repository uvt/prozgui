...
/* $Id: ftp.c,v 1.43 2001/09/23 01:39:15 kalum Exp $ */
/* $Id: ftp.c,v 1.44 2011/03/24 01:39:15 ahmad Exp $ */
...
/******************************************************************************
 ...
******************************************************************************/
uerr_t ftp_rest(connection_t * connection, long long bytes)
{
  uerr_t err;
...
******************************************************************************/
uerr_t ftp_size(connection_t * connection, const char *file, long long *size)
{
  uerr_t err;
		 ...
		 const char *passwd)
{
  ...
  while (1)
  {
    ...
    default:
      /* Unknown error code. */
      proz_debug(_("Unknown code %d returned during FTP login"), ret_code);
      return FTPERR;
      break;
    }
...
*/
  ...
  if (err != FTPPARSEOK)
  {
    connection_show_message(connection,
			    _
			    ("Unable to parse the line the FTP server returned:please report URL to raviyanto@gmail.com "));
  }

  if(err==FTPPARSEOK)
    {
      proz_debug("size returned from LIST %ld",fp.filesize);
      //SEC size_rt long long?
      if(size_ok==FALSE)
	{
			     ...
			     boolean * passive_mode)
{
  ...
  if (connection->ftp_use_pasv == TRUE)
  {
    ...
    {
      proz_debug(_("Server does not seem to support PASV"));
      *passive_mode = FALSE;
